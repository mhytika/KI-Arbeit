...
document.addEventListener('DOMContentLoaded', () => {
     ...
     * Shows warning overlay if in portrait mode.
     */
    function updateOrientationState() {
        const width = window.innerWidth;
        const height = window.innerHeight;

        // Check if portrait (width < height)
        if (width < height) {
            // Show warning, hide game
            orientationWarning.classList.remove('hidden');
            gameShell.classList.add('hidden');
        } else {
            // Show game, hide warning
    ...
    function renderQuestion() {
        ...
        Object.entries(questionData.options).forEach(([key, text]) => {
            ...
            if (questionContainer && questionContainer.dataset.scored) {
        ...
        // Footer Buttons state based on scored
    ...
    function checkQuizAnswer() {
        ...
        if (questionContainer) {
            if (isCorrect) {
                questionContainer.classList.add('correct-state');
            } else {
                questionContainer.classList.add('incorrect-state');
            }
        }
        ...
        if (questionContainer && !questionContainer.dataset.scored) {
            if (isCorrect && window.gameState && window.gameState.scores) {
                window.gameState.scores.quiz = (window.gameState.scores.quiz || 0) + 1;
            }
            questionContainer.dataset.scored = 'true';
        }
    }
